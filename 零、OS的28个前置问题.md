...
小端字节序是高位放在低地址处、低位放在高地址。

大端字节序是高位放在高地址、低位放在低地址。

它们各有优点：

小端字节序的优点：

* 便于强制类型转换，因为强制类型转换时，会截断高地址的处保存的值，小端字节序的高地址处存储的是低位。比如说我用小端字节序存储0x12 34 56 78。78存储在高地址处，在强制类型转换时，高地址处的值被截断，78刚好被截断了

大端字节序的优点：

* 便于获取符号位，因为大端字节序是低位存在低地址，当CPU读取低地址内容的时候，可以直接获取符号位，而不用等几个字节后，才能获取到符号位
